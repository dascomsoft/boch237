...
const ChatWindow = ({
  ...
  useEffect(() => {
    ...
    socket.on('message_edited', (data: { messageId: string; content: string }) => {
      console.log('✏️ Message modifié:', data);
      if (onEditMessage) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-slate-900 pb-16">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {sortedMessages.length === 0 && (
          <div className="flex items-center justify-center h-32">
            <p className="text-gray-500 text-sm text-center">💬 Aucun message encore.<br />Commencez la conversation !</p>
          </div>
        )}
        ...
        {sortedMessages.map((msg) => (
          <div
            key={msg._id}
            className={`flex ${msg.senderId === currentUserId ? 'justify-end' : 'justify-start'} relative group`}
          >
            {editingMessage && editingMessage.id === msg._id ? (
              <div className="flex items-center gap-2 w-full max-w-[75%]">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-lg bg-slate-800 text-white border border-green-500 focus:outline-none"
                />
                <button onClick={saveEditMessage} className="bg-green-600 px-3 py-2 rounded-lg text-white">✓</button>
                <button onClick={cancelEdit} className="bg-gray-600 px-3 py-2 rounded-lg text-white">✗</button>
              </div>
            ) : (
              <div
              ...
              >
                <p className="break-words text-sm md:text-base leading-relaxed pr-6">{msg.content}</p>
                ...
                {showMenu === msg._id && msg.senderId === currentUserId && (
                  <div className="absolute -top-10 right-0 bg-slate-800 rounded-lg shadow-xl flex overflow-hidden z-20">
                    <button onClick={() => startEditMessage(msg)} className="px-3 py-1.5 text-xs text-white hover:bg-slate-700 flex items-center gap-1">
                      ✏️ Modifier
                    </button>
                    <button onClick={() => handleDeleteMessage(msg._id!)} className="px-3 py-1.5 text-xs text-red-400 hover:bg-slate-700 flex items-center gap-1">
                      🗑️ Supprimer
                    </button>
                  </div>
                )}
      ...
      <div className="flex-shrink-0 bg-slate-800 px-3 py-2 border-t border-slate-700">
        <div className="flex gap-2 items-center">
          <input
            ref={inputRef}
            type="text"
            className="flex-1 min-w-0 px-4 py-3 rounded-full bg-slate-900 text-white border border-slate-600 focus:outline-none focus:border-green-500 text-sm"
            value={input}
            onChange={(e) => setInput(e.target.value)}
          ...
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="w-10 h-10 rounded-full bg-green-600 hover:bg-green-700 text-white flex items-center justify-center disabled:opacity-50"
          >
