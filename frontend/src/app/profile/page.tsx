  ...
  const fetchData = async (token: string) => {
    try {
      const [userRes, convRes] = await Promise.all([
        ...
        axios.get('http://localhost:5001/api/users/conversations', {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
      ...
      <div className="p-4 space-y-4">
        ...
        {user?.role === 'parent' && (
          <div className="bg-gradient-to-r from-pink-600/20 to-rose-600/20 border border-pink-500 rounded-xl p-4">
            ...
            >
              <Megaphone size={20} />
              Publier une annonce de besoin
              <PlusCircle size={16} />
            </button>
            <p className="text-gray-400 text-xs text-center mt-2">
              Besoin d'un répétiteur ? Publiez une annonce gratuitement
            </p>
          </div>
        )}
        ...
        <button
          ...
          className="w-full bg-red-600 hover:bg-red-700 text-white p-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors"
        >
